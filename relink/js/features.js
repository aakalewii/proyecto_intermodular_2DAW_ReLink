(() => {
  // Reutilizamos el namespace global ReLink creado en core.js
  ...
  // Asegura que los usuarios de demo existan en localStorage
  RL.ensureDemoUsers = () => {
    const users = RL.loadUsers();
    let changed = false;

    for (const u of RL.DEMO_USERS) {
      // Si no existe un usuario con ese email, lo añadimos
      if (!users.some((x) => x.email === u.email)) {
        users.push({
          ...u,
          // Fecha de creación aleatoria en el pasado
  ...
  // Actualiza la UI según si hay usuario logueado o no
  RL.setAuthUI = (me) => {
    const guest = RL.$("#authGuest");
    const user = RL.$("#authUser");
    const chip = RL.$("#userChip");

    // Chip con el nombre del usuario o icono genérico
    if (chip) {
    ...
    // Muestra/oculta bloques según haya sesión o no
  ...
  // Seed de 12 anuncios de demo si no hay ninguno
  RL.ensureDemoAds = () => {
    const existing = RL.loadAds();
    if (existing && existing.length) return;

    // Genera una imagen SVG en base64 para cada anuncio
  ...
  // Abre un modal <dialog> por ID
  RL.openModal = (id) => {
    const d = RL.$("#modal-" + id);
    if (!d) return;

    // ShowModal() es propio de <dialog>, no todos los alumnos lo usan
    if (typeof d.showModal === "function") d.showModal();
    else d.setAttribute("open", "open"); // fallback si el navegador no soporta <dialog>
  };
  ...
  // Cierra modal haciendo clic fuera del cuadro
  RL.initModalOutsideClick = () => {
    RL.$$("dialog.modal").forEach((d) => {
      d.addEventListener("click", (e) => {
        const r = d.getBoundingClientRect();

        // Detecta si el clic fue dentro del modal
        const inside =
          e.clientX >= r.left && e.clientX <= r.right &&
          e.clientY >= r.top  && e.clientY <= r.bottom;

        // Si fue fuera → cerrar modal
    ...
    RL.initGlobalClicks = () => {
      document.addEventListener("click", (e) => {
        ...
        if (open) {
          ...
          // Si intenta publicar sin sesión → abrir login
          if (id === "post" && !RL.getMe()) {
    ...
    // Cambia entre pestañas "login" y "register"
    RL.setAuthTab = (which) => {
    ...
    // Inicializa formularios de login, registro y logout
    RL.initAuthForms = () => {
      const authModal = RL.$("#modal-auth");
      if (!authModal) return;
    ...
    // =========================
  // Post form (crear/editar)
  // =========================

  // Variable global para saber si estamos editando un anuncio existente
  let editingAdId = null;

  /* Lee archivos como base64
    ------------------------------------------------
    Permite convertir imágenes a dataURL para guardarlas en localStorage.
  */
  ...
  /* Rellena el formulario de publicar/editar con los datos del anuncio */
  ...
  RL.openEditAd = (adId) => {
    ...
    const dict = RL.I18N?.[RL.getLang?.() || "es"] || RL.I18N?.es || {};
    if (!me)
      return RL.toast(dict["toast.signinFirst"] || "Primero inicia sesión", "warn");
  ...
  RL.deleteAd = (adId) => {
    ...
    // Quitar de favoritos y descartados
  ...
  /* Inicializar formulario de publicar/editar */
  RL.initPostForm = () => {
    ...
    postForm.addEventListener("submit", async (e) => {
      ...
      const me = RL.getMe();
      if (!me) {
        RL.toast(dict["toast.signinToPublish"] || "Primero inicia sesión para publicar", "warn");
        RL.openModal("auth");
        RL.setAuthTab("login");
        return;
      }
  ...
  /* Crea el modal de producto si no existe */
  RL.ensureProductModal = () => {
    ...
    /* Genera HTML dinámico para un modal completo */
    d.innerHTML = `
  ...
  /* Mostrar modal de producto con datos reales */
  RL.showProductModal = (adId) => {
    ...
    // Rellenar datos
    if (titleEl) titleEl.textContent = ad.title || dict["product.title"] || "Anuncio";
    if (priceEl) priceEl.textContent = `${Number(ad.price || 0).toFixed(0)} €`;
    if (metaEl) metaEl.textContent = `${ad.loc || "—"} · ${dict.today || "Hoy"}`;
    ...
    if (sellerEl) {
      ...
      sellerEl.href = `perfil.html?u=${encodeURIComponent(ad.ownerEmail || "")}`;
    }
    ...
    if (mediaEl) {
      ...
      mediaEl.innerHTML = img
        ? `<div class="product__img" style="background:url('${img.replace(/'/g, "%27")}') center/cover no-repeat;"></div>`
    ...
    btnBuy && (btnBuy.onclick = () =>
      RL.toast(dict["toast.buy"] || "Simulación de compra (demo)", "ok")
    );

    btnContact && (btnContact.onclick = () =>
      RL.toast(dict["toast.contact"] || "Abriendo contacto…", "info")
    );

    btnReport && (btnReport.onclick = () =>
      RL.toast(dict["toast.reported"] || "Gracias, revisaremos el anuncio", "ok")
    );
